...
public class MemberController {
	...
	public void joinProcess(MemberDto memberDto,HttpServletRequest request, HttpServletResponse response,MultipartFile multipartFile) throws Exception {
		//MultipartFile (join에 만들어둔 name) - 파일 업로드
		String context = request.getContextPath(); // 현재 돌아가는 것
		String imgFolder =  "C:\\galleryImage\\"; 
		String originalFileName = multipartFile.getOriginalFilename();
		
		String extention = FilenameUtils.getExtension(originalFileName); // 확장자 구하기...
		
		String savedFileName = UUID.randomUUID()+"."+extention;   // 랜덤한 값을 16가지 뽑아준다
		File targetFile = new File(imgFolder+savedFileName); // galleryImage 폴더에 던져 주기(file은 io에 있는 것 가져오기)
		String dbFileName = context+"/galleryImage/"+savedFileName; // db에 집어 넣을때 
		...
		InputStream fileStream = multipartFile.getInputStream();
		FileUtils.copyInputStreamToFile(fileStream,targetFile);//fileStream 을 읽어서 targetFile로 전달 하겠다
		...
		// 섬네일 메이븐에서 받아온 후 한줄 입력  
		// 섬네일 크기 조정 gallery 에 보낼때 크기 설정
		Thumbnails.of(targetFile).size(100,100).outputFormat(extention).toFile(thumbTargetFile); 
